...
| Java             | Shape              | single-file Java records and unit enums        | exactly one public root record or unit enum; arrays, `List<T>`, `Map<String, T>`, references, and recursive records; no classes, annotations, data-carrying enums, generics, or JavaBeans |
